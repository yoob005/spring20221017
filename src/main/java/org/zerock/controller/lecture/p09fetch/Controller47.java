...
public class Controller47 {
	...
	public Map<String, String> method10(@RequestBody JavaBean18 customer) {
		...
		if(cnt == 1) {
			...
			map.put("message", customer.getId() + "번 고객정보가 변경 ㄴㄴㄴㄴ.");
		}
	...
	public Map<String, String> method11(@RequestBody JavaBean19 supplier) {
		...
		if(cnt == 1) {
			map.put("message", supplier.getId() + "번 공급자 정보가 변경되었습니다.");
		} else {
			map.put("message", supplier.getId() + "번 공급자 정보가 변경 ㄴㄴㄴㄴ.");
		}
